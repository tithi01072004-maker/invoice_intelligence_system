...
st.set_page_config(
    page_title="InvoiceGuard: Intelligent Invoice Fraud Detection Portal",
    page_icon="🧾",
    layout='wide'
)
...
st.markdown(
    """
# 🧾InvoiceGuard: Intelligent Invoice Fraud Detection Portal
...
""")
    ...
    if submit_freight:
        ...
        st.session_state['freight_result']=prediction[0]

        st.metric(
            label="💰 📊  Estimated Freight Cost",
              value=f"${st.session_state['freight_result']:.2f}"
        )
...
""")
    ...
    with st.form("invoice_flag_form",clear_on_submit=False):
